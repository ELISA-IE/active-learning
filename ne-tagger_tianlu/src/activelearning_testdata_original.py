...
class ActiveLearning(object):
    ...
    def __init__(self, init_train_num=50, increment=10, work_dir=None, total_train_sentences=100):
        ...
        # print 'just to check that the test_set is normal: test_set[1214] is ' + self.test_set[1214] # for debug
        #########

        self.init_train_num = int(init_train_num)
        self.increment = int(increment)
        print 'self.init_train_num is :'+ str(self.init_train_num)
	print 'self.increment is:' + str(self.increment)
        self.init_train_set = []
    ...
    def do_training(self, sampling_method):
        ...
        while(len(self.current_train_set)<=self.total_sentences):
            ...
            print '--------------------------------------begin tag in do training---------------------------------------'
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print 'USAGE: ./src/activelearning_testdata_original.py <work dir> <sampling method(segment_entropy_sampling or random_sampling)> ' \
